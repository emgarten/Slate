...
{
    /// <summary>
    /// Utilities for running tasks in parallel with throttling.
    /// </summary>
    public static class TaskUtils
    {
        ...
        /// Run tasks in parallel.
        /// </summary>
        public static Task RunAsync(IEnumerable<Func<Task>> tasks, int maxThreads)
        {
            return RunAsync(
                tasks: tasks.Select(GetFuncWithReturnValue),
        ...
        /// Run tasks in parallel.
        /// </summary>
        public static Task RunAsync(IEnumerable<Func<Task>> tasks, int maxThreads, CancellationToken token)
        {
            return RunAsync(
                tasks: tasks.Select(GetFuncWithReturnValue),
                maxThreads: maxThreads,
        ...
        /// Run tasks in parallel.
        /// </summary>
        public static Task RunAsync(IEnumerable<Func<Task>> tasks, int maxThreads, bool useTaskRun)
        {
            return RunAsync(
                tasks: tasks.Select(GetFuncWithReturnValue),
                maxThreads: maxThreads,
        ...
        /// Run tasks in parallel.
        /// </summary>
        public static Task RunAsync(IEnumerable<Func<Task>> tasks, int maxThreads, bool useTaskRun, CancellationToken token)
        {
            return RunAsync(
                tasks: tasks.Select(GetFuncWithReturnValue),
                maxThreads: maxThreads,
        ...
        public static Task<T[]> RunAsync<T>(IEnumerable<Func<Task<T>>> tasks, bool useTaskRun, CancellationToken token)
        {
            ...
            return RunAsync(tasks, maxThreads, useTaskRun, token);
        }
        ...
        {
            return RunAsync(tasks, maxThreads, useTaskRun: false, token: CancellationToken.None);
        }

        /// <summary>
        /// Run tasks in parallel and returns the results in the original order.
        /// </summary>
        public static Task<T[]> RunAsync<T>(IEnumerable<Func<Task<T>>> tasks, int maxThreads, CancellationToken token)
        {
            return RunAsync(tasks, maxThreads, useTaskRun: false, process: DefaultProcess, token: token);
        }

        /// <summary>
        /// Run tasks in parallel and returns the results in the original order.
        /// </summary>
        public static Task<T[]> RunAsync<T>(IEnumerable<Func<Task<T>>> tasks, int maxThreads, bool useTaskRun)
        {
            return RunAsync(tasks, maxThreads, useTaskRun, CancellationToken.None);
        }

        /// <summary>
        /// Run tasks in parallel and returns the results in the original order.
        /// </summary>
        public static Task<T[]> RunAsync<T>(IEnumerable<Func<Task<T>>> tasks, int maxThreads, bool useTaskRun, CancellationToken token)
        {
            return RunAsync(tasks, maxThreads, useTaskRun, DefaultProcess, token);
        }

        /// <summary>
        /// Run tasks in parallel and returns the results in the original order.
        /// </summary>
        public async static Task<R[]> RunAsync<T, R>(IEnumerable<Func<Task<T>>> tasks, int maxThreads, bool useTaskRun, Func<Task<T>, Task<R>> process, CancellationToken token)
        {
